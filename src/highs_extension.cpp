...
struct HighsModelInfo {
    ...
    std::vector<std::vector<std::pair<int, double>>> constraint_coefficients; // [constraint_idx][{var_idx, coeff}]
...
void HighsExtension::Load(ExtensionLoader &loader) {
	LoadInternal(loader);
}
...
struct HighsCreateVariablesFunction {
	static void CreateVariablesFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
		...
		auto* model_info = HighsModelRegistry::Instance().GetOrCreateModel(bind_data.model_name);
		
		try {
			// Check if variable already exists
			if (model_info->variable_indices.find(bind_data.variable_name) != model_info->variable_indices.end()) {
				throw std::runtime_error("Variable '" + bind_data.variable_name + "' already exists in model '" + bind_data.model_name + "'");
			}
			...
			std::string index_str = bind_data.variable_name + "_" + std::to_string(var_index);
			variable_name_vector[0] = StringVector::AddString(output.data[0], bind_data.variable_name);
			variable_index_vector[0] = StringVector::AddString(output.data[1], index_str);
			...
			variable_name_vector[0] = StringVector::AddString(output.data[0], bind_data.variable_name);
			variable_index_vector[0] = StringVector::AddString(output.data[1], "ERROR"); // Error case
			status_vector[0] = StringVector::AddString(output.data[2], "ERROR: " + std::string(e.what()));
		}
		
		global_state.finished = true;
	}
	
	static unique_ptr<FunctionData> CreateVariablesBind(ClientContext &context, TableFunctionBindInput &input,
		...
		if (input.inputs.size() != 6) {
			throw BinderException("highs_create_variables expects exactly 6 parameters: model_name, variable_name, lower_bound, upper_bound, obj_coefficient, var_type");
		}
	...
	static unique_ptr<GlobalTableFunctionState> CreateVariablesInit(ClientContext &context, TableFunctionInitInput &input) {
...
struct HighsCreateConstraintsFunction {
	static void CreateConstraintsFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
		...
		try {
			...
			if (model_info->constraint_indices.find(bind_data.constraint_name) != model_info->constraint_indices.end()) {
				throw std::runtime_error("Constraint '" + bind_data.constraint_name + "' already exists in model '" + bind_data.model_name + "'");
			}
			
			// Store constraint info
			int constraint_index = model_info->next_constraint_index++;
			model_info->constraint_indices[bind_data.constraint_name] = constraint_index;
			...
			model_info->constraint_coefficients.push_back(std::vector<std::pair<int, double>>());
			...
			// Set output
			output.SetCardinality(1);
			auto constraint_name_vector = FlatVector::GetData<string_t>(output.data[0]);
			auto constraint_index_vector = FlatVector::GetData<string_t>(output.data[1]);
			auto status_vector = FlatVector::GetData<string_t>(output.data[2]);
			
			std::string index_str = bind_data.constraint_name + "_" + std::to_string(constraint_index);
			constraint_name_vector[0] = StringVector::AddString(output.data[0], bind_data.constraint_name);
			constraint_index_vector[0] = StringVector::AddString(output.data[1], index_str);
			status_vector[0] = StringVector::AddString(output.data[2], "SUCCESS");
			...
			constraint_name_vector[0] = StringVector::AddString(output.data[0], bind_data.constraint_name);
			constraint_index_vector[0] = StringVector::AddString(output.data[1], "ERROR"); // Error case
			status_vector[0] = StringVector::AddString(output.data[2], "ERROR: " + std::string(e.what()));
		}
	...
	static unique_ptr<FunctionData> CreateConstraintsBind(ClientContext &context, TableFunctionBindInput &input,
	                                                       vector<LogicalType> &return_types, vector<string> &names) {
		auto result = make_uniq<HighsCreateConstraintsData>();
		
		// Extract parameters from input
		if (input.inputs.size() != 4) {
			throw BinderException("highs_create_constraints expects exactly 4 parameters: model_name, constraint_name, lower_bound, upper_bound");
		}
	...
	static unique_ptr<GlobalTableFunctionState> CreateConstraintsInit(ClientContext &context, TableFunctionInitInput &input) {
		return make_uniq<SingleRowGlobalState>();
	}
};
...
struct HighsSetCoefficientsFunction {
	static void SetCoefficientsFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
		...
		// Get model from registry
		auto* model_info = HighsModelRegistry::Instance().GetModel(bind_data.model_name);
		if (!model_info) {
			output.SetCardinality(1);
			auto constraint_name_vector = FlatVector::GetData<string_t>(output.data[0]);
			...
			constraint_name_vector[0] = StringVector::AddString(output.data[0], bind_data.constraint_name);
			variable_name_vector[0] = StringVector::AddString(output.data[1], bind_data.variable_name);
			coefficient_vector[0] = bind_data.coefficient;
			status_vector[0] = StringVector::AddString(output.data[3], "ERROR: Model '" + bind_data.model_name + "' not found");
			global_state.finished = true;
			return;
		}
		...
		try {
			...
			auto constraint_it = model_info->constraint_indices.find(bind_data.constraint_name);
			
			if (var_it == model_info->variable_indices.end()) {
				throw std::runtime_error("Variable '" + bind_data.variable_name + "' not found in model '" + bind_data.model_name + "'");
			}
			
			if (constraint_it == model_info->constraint_indices.end()) {
				throw std::runtime_error("Constraint '" + bind_data.constraint_name + "' not found in model '" + bind_data.model_name + "'");
			}
			...
			model_info->constraint_coefficients[constraint_index].push_back({var_index, bind_data.coefficient});
			...
			variable_name_vector[0] = StringVector::AddString(output.data[1], bind_data.variable_name);
			coefficient_vector[0] = bind_data.coefficient;
			status_vector[0] = StringVector::AddString(output.data[3], "ERROR: " + std::string(e.what()));
		}
		
		global_state.finished = true;
	}
	
	static unique_ptr<FunctionData> SetCoefficientsBind(ClientContext &context, TableFunctionBindInput &input,
	                                                     vector<LogicalType> &return_types, vector<string> &names) {
		auto result = make_uniq<HighsSetCoefficientsData>();
		
		// Extract parameters from input
		if (input.inputs.size() != 4) {
			throw BinderException("highs_set_coefficients expects exactly 4 parameters: model_name, constraint_name, variable_name, coefficient");
		}
	...
	static unique_ptr<GlobalTableFunctionState> SetCoefficientsInit(ClientContext &context, TableFunctionInitInput &input) {
		return make_uniq<SingleRowGlobalState>();
	}
};
...
struct HighsSolveFunction {
	static void SolveFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
		...
		if (!model_info) {
			...
			variable_index_vector[0] = StringVector::AddString(output.data[1], "ERROR");
			solution_value_vector[0] = 0.0;
			reduced_cost_vector[0] = 0.0;
			status_vector[0] = StringVector::AddString(output.data[4], "ERROR: Model '" + bind_data.model_name + "' not found");
			return;
		}
			...
			try {
				...
				for (int i = 0; i < model_info->next_var_index; i++) {
					...
					if (var_type == "binary") {
						...
						model_info->model.lp_.col_lower_[i] = std::max(0.0, model_info->model.lp_.col_lower_[i]);
						model_info->model.lp_.col_upper_[i] = std::min(1.0, model_info->model.lp_.col_upper_[i]);
				...
				solution_value_vector[0] = 0.0;
				reduced_cost_vector[0] = 0.0;
				status_vector[0] = StringVector::AddString(output.data[4], "ERROR: " + std::string(e.what()));
				return;
			}
		}
		...
		idx_t batch_size = std::min(num_variables - current_row, (idx_t)STANDARD_VECTOR_SIZE);
		...
		for (idx_t i = 0; i < batch_size; i++) {
			...
			variable_name_vector[i] = StringVector::AddString(output.data[0], var_name);
			variable_index_vector[i] = StringVector::AddString(output.data[1], index_str);
			solution_value_vector[i] = global_state.solution_values.size() > var_idx ? global_state.solution_values[var_idx] : 0.0;
			reduced_cost_vector[i] = global_state.reduced_costs.size() > var_idx ? global_state.reduced_costs[var_idx] : 0.0;
	...
	static unique_ptr<FunctionData> SolveBind(ClientContext &context, TableFunctionBindInput &input,
	                                          vector<LogicalType> &return_types, vector<string> &names) {
		auto result = make_uniq<HighsSolveData>();
		
		// Extract parameters from input
		if (input.inputs.size() != 1) {
			throw BinderException("highs_solve expects exactly 1 parameter: model_name");
		}
	...
	static unique_ptr<GlobalTableFunctionState> SolveInit(ClientContext &context, TableFunctionInitInput &input) {
...
void LoadInternal(ExtensionLoader &loader) {
	...
	// highs_create_variables(model_name, variable_name, lower_bound, upper_bound, obj_coefficient, var_type)
	TableFunction create_variables_function("highs_create_variables", {LogicalType::VARCHAR, LogicalType::VARCHAR, LogicalType::DOUBLE, LogicalType::DOUBLE, LogicalType::DOUBLE, LogicalType::VARCHAR}, 
	                                        HighsCreateVariablesFunction::CreateVariablesFunction, HighsCreateVariablesFunction::CreateVariablesBind, HighsCreateVariablesFunction::CreateVariablesInit);
	loader.RegisterFunction(create_variables_function);
	
	// highs_create_constraints(model_name, constraint_name, lower_bound, upper_bound)
	TableFunction create_constraints_function("highs_create_constraints", {LogicalType::VARCHAR, LogicalType::VARCHAR, LogicalType::DOUBLE, LogicalType::DOUBLE}, 
	                                          HighsCreateConstraintsFunction::CreateConstraintsFunction, HighsCreateConstraintsFunction::CreateConstraintsBind, HighsCreateConstraintsFunction::CreateConstraintsInit);
	loader.RegisterFunction(create_constraints_function);
	
	// highs_set_coefficients(model_name, constraint_name, variable_name, coefficient)
	TableFunction set_coefficients_function("highs_set_coefficients", {LogicalType::VARCHAR, LogicalType::VARCHAR, LogicalType::VARCHAR, LogicalType::DOUBLE}, 
	                                        HighsSetCoefficientsFunction::SetCoefficientsFunction, HighsSetCoefficientsFunction::SetCoefficientsBind, HighsSetCoefficientsFunction::SetCoefficientsInit);
	loader.RegisterFunction(set_coefficients_function);
	
	// highs_solve(model_name)
	TableFunction solve_function("highs_solve", {LogicalType::VARCHAR}, 
	                             HighsSolveFunction::SolveFunction, HighsSolveFunction::SolveBind, HighsSolveFunction::SolveInit);
...
DUCKDB_CPP_EXTENSION_ENTRY(highs, loader) {
